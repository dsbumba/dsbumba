...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 26 19:16:31 2021
...
for no_episode in range(max_episodes):
    ...
    current_action = epsilon_greedy_policy(current_state, explore_rate)
    episode_rewards = 0
    for time_step in range(max_time_steps):
        env.render()
        new_state, reward, complete, _ = env.step(current_action)
        new_state = discretize(new_state)
        new_action = epsilon_greedy_policy(new_state, explore_rate)
        Q_value[current_state][current_action] += learning_rate * (reward + discount* Q_value[new_state][new_action] - Q_value[current_state][current_action])
